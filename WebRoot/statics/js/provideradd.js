...
$(function(){
	...
	//证件照片
	a_idPicPath = $("#a_companyLicPicPath");
	errorinfo = $("#errorinfo");
	if(errorinfo.val()==null||errorinfo.val()=="")
	{
		a_idPicPath.next().html("*上传文件大小不能超过500KB*上传文件类型必须为：jpg、jpeg、png、pneg");
	}else
	{
		a_idPicPath.next().html(errorinfo.val());
	}
	//工作证照片
	a_workPicPath = $("#a_orgCodePicPath");
	errorinfo_wp = $("#errorinfo_wp");
	if(errorinfo_wp.val()==null||errorinfo_wp.val()=="")
	{
		a_workPicPath.next().html("*上传文件大小不能超过500KB*上传文件类型必须为：jpg、jpeg、png、pneg");
	}else
	{
